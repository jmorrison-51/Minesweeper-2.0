...
- Only one web tab plays at a time (the desktop's single-instance mutex, for the browser), because each tab keeps its own `SaveData` and the last save would win. The playing tab holds a Web Lock (`ms2.tab` in `ms2.js`, `GameSession.Tab`); another tab shows "Play here instead", which asks the playing tab over a `BroadcastChannel` to save and let go (`OnYield`, which raises `Yielding` so Endless records its run), steals the lock after 3 s if it gets no answer, and reloads the save. `Persist` does nothing unless this tab is playing, since screens save as they close. Browsers without Web Locks (or pages on plain `http` other than localhost, which are not a secure context) are not guarded.
