...
- `src/Minesweeper.Core` is a UI-free class library (rules, state, persistence). It must not reference WinForms or any rendering code, because future hex, mobile and web front ends are meant to reuse it.
  ...
  - `Board.Neighbors` hardcodes 8-way square adjacency, and mine placement and counting also assume a square grid. Hex mode needs this generalized (for example a topology abstraction) rather than a parallel `Board`.
...
- `src/Minesweeper.Desktop` is the WinForms front end. It has no designer files and all UI is built in code.
  - `Program` loops: `StartForm` (mode picker, sets `Selected`) then the chosen game form. `MainForm.ReturnToMenu` (Game > Main Menu) sends the user back to the start screen. `GameMode.Hex` and `GameMode.HexChallenge` currently just show a "coming soon" message in `Program`; wire the real forms in that switch.
  - `MainForm` is the classic-game coordinator. It builds the menu, owns the `Board`, the stopwatch and timer, and the save file (`%AppData%\Minesweeper2\save.json`), and lays out the header and board manually (DPI-scaled with `LogicalToDeviceUnits`). It draws the sunken bevels itself in `OnPaint`.
  - `BoardControl` is a custom-painted control that draws cells with GDI+ and translates mouse input (including chord and pressed-cell visuals) into `Board` calls. It raises `Changed` after each input and `PressingChanged` for the face button.
