...
- `src/Minesweeper.Core` is a UI-free class library (rules, state, persistence). It must not reference WinForms or any rendering code, because future hex, mobile and web front ends are meant to reuse it.
  ...
  - Profiles: `ProfileStore` keeps one encrypted save per player in `%AppData%\Minesweeper2\profiles\p_<name>.dat` (the `p_` prefix avoids reserved Windows names; names are 1-20 letters/digits/space/-/_, unique ignoring case) plus `last.txt` for the last player. The old single `save.json` is migrated once into a player called "Player" (`MigrateLegacy`, renamed to `save.json.migrated`). `Program` holds the signed-in player (`CurrentProfile`, `IsAdmin`, `SavePath`, `LoadSave()`); every form loads and saves through those. `ProfileForm` ("Who's playing?", shown at startup and from the start screen's Switch Player) picks, creates (`NewPlayerDialog`) or deletes players.
  - High scores: `Leaderboard` (Core, pure functions over `ProfileStore.LoadAll()`) ranks every listed player, best first, per mode: fastest time per difficulty for classic and hex (`Difficulty.Key`), furthest Hex Challenge level (`LevelsCleared`, ties by flagless clears), longest Endless run (ties by rows). The admin login is never listed. `StartForm` shows an owner-drawn `ListBox` beside each mode (current player in blue bold, native scrollbar when there are more players than fit) and a "Times: <difficulty>" button that cycles which difficulty the classic and hex lists show. `StartForm` accepts a player list so it can be rendered with sample data.
  - Admin login: typing the name `admin` in the New Player dialog reveals a password box; the right password (`AdminAccess`, stored only as a SHA-256 hash) signs in to a separate, unlisted `admin` profile whose `SaveData.AdminUnlock` (runtime only, never saved) unlocks Endless Mode for testing. `admin` cannot be created as a normal player. To change the password, replace the hash in `AdminAccess.cs` (the file has the PowerShell one-liner).
  ...
  - `SaveData` is JSON persistence (last difficulty per shape, custom-field settings, tile size 24/36/48 chosen on the start screen, best times in ms per difficulty key). It takes a file path, so Core stays platform-neutral. Files are encrypted (`SaveCrypto`: AES-CBC plus an HMAC, keys derived from a phrase inside the program) so they cannot be edited in a text editor; an edited, corrupt or plain-text file fails the HMAC, is copied to `<name>.invalid`, and the player starts fresh. Only `LoadAny` (used for the one-time upgrade from the old plain `save.json`) accepts plain JSON. Saves go through a temp file then a swap.
- `src/Minesweeper.Desktop` is the WinForms front end. It has no designer files and all UI is built in code.
  ...
  - `MainForm` is the coordinator for classic and hex sandbox play. It builds the menu, owns the `Board`, the stopwatch and timer, and the signed-in player's save (`Program.SavePath`, see Profiles below), and lays out the header and board manually (DPI-scaled with `LogicalToDeviceUnits`). It draws the sunken bevels itself in `OnPaint`.
